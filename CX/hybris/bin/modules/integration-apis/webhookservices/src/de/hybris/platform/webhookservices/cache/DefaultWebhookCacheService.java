...
public class DefaultWebhookCacheService implements WebhookCacheService
{
	...
	private static final Logger LOG = Log.getLogger(DefaultWebhookCacheService.class);
	private static final String ITEM_NOT_FOUND_MESSAGE = "Could not find the item {} with PK: [{}]";
	private static final String ITEM_NOT_CLONED_MESSAGE = "Could not clone the item {} with PK: [{}]";
	...
	private void cacheItem(final ItemModel item, final InterceptorContext ctx)
	{
		...
		try
		{
			...
			final ItemModel clonedItem = modelService.clone(modelService.get(item.getPk()));
		...
		{
			LOG.debug(ITEM_NOT_FOUND_MESSAGE, item.getItemtype(), item.getPk());
		}
		catch (final JaloInternalException e)
		{
			LOG.debug(ITEM_NOT_CLONED_MESSAGE, item.getItemtype(), item.getPk());
		}
	}
	                                    ...
	                                    final IntegrationCache<WebhookDeletedItemCacheKey, T> cache)
	{
		getCurrentTransaction().executeOnRollback(new Transaction.TransactionAwareExecution()
		{
			...
			public Object getId()
			{
				...
				return key.getId() + DELIMITER + (Objects.nonNull(value) ? value.getClass().getSimpleName() : "") + DELIMITER;
			}
		});
	}
