...
import com.hybris.merchandising.jalo.MerchSynchronization;
...
public abstract class GeneratedMerchandisingservicesManager extends Extension
{
	...
	public MerchSynchronization createMerchSynchronization(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( MerchandisingservicesConstants.TC.MERCHSYNCHRONIZATION );
			return (MerchSynchronization)type.newInstance( ctx, attributeValues );
		}
		catch( JaloGenericCreationException e)
		{
			final Throwable cause = e.getCause();
		...
		{
			throw new JaloSystemException( e ,"error creating MerchSynchronization : "+e.getMessage(), 0 );
		}
	}
	
	public MerchSynchronization createMerchSynchronization(final Map attributeValues)
	{
		return createMerchSynchronization( getSession().getSessionContext(), attributeValues );
	}
