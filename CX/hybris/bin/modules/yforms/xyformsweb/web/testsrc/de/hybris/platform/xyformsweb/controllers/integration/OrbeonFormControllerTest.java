...
public class OrbeonFormControllerTest
{
	...
	public void shouldResponseWithOkeyWithCreateSuccessed() throws YFormServiceException, IOException, ServletException
	{
		...
		yFormData.setVersion(1);
		yFormData.setLastModifiedDate(new Date());

		final ServletInputStream inputStream = new DelegatingServletInputStream(IOUtils.toInputStream(newFormDefinition, "UTF-8"));
		Mockito.lenient().when(httpRequest.getInputStream()).thenReturn(inputStream);
		Mockito.lenient().when(httpRequest.getHeader(ORBEON_FORM_DEFINITION_VERSION)).thenReturn("1");
		Mockito.lenient().when(yFormFacade.createOrUpdateYFormDefinition(applicationId, formId, newFormDefinition, null, "1")).thenReturn(yFormData);

		controller.putFormDefinition(applicationId, formId, null, httpRequest, httpResponse);
		verify(yFormFacade, times(1)).createOrUpdateYFormDefinition(anyString(), anyString(), anyString(), nullable(String.class), anyString());
		verify(httpRequest, times(1)).getInputStream();
		verify(httpResponse, times(1)).setStatus(HttpServletResponse.SC_OK);
		verify(httpResponse, times(1)).setHeader(ORBEON_FORM_DEFINITION_VERSION, "1");
	}
	...
	public void shouldReturnOkWhenSuccessCreateOrUpdatedYFormData() throws IOException, YFormServiceException, ServletException
	{
		final String newFormDefinition = "";
		final YFormDefinitionData yFormDefinition = new YFormDefinitionData();
		yFormDefinition.setVersion(1);

		final YFormDataData yFormDataData = new YFormDataData();
		yFormDataData.setLastModifiedDate(new Date());
		yFormDataData.setFormDefinition(yFormDefinition);

		final ServletInputStream inputStream = new DelegatingServletInputStream(IOUtils.toInputStream(newFormDefinition, "UTF-8"));
		Mockito.lenient().when(httpRequest.getInputStream()).thenReturn(inputStream);
		Mockito.lenient().when(yFormFacade.createOrUpdateYFormData(applicationId, formId, formDataId, YFormDataTypeEnum.DATA,
				newFormDefinition)).thenReturn(yFormDataData);
