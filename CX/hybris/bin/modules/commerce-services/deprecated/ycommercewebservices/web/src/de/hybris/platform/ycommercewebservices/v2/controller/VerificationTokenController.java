...
public class VerificationTokenController extends BaseController {
    ...
    @ApiOperation(nickname = "createVerificationToken", value = "Creates verification token", notes = "Creates verification token, and sends token code via designated channel e.g. email.")
    public VerificationTokenWsDTO createVerificationToken(
            @ApiParam(value = "Object contains information for obtaining a verification token.", required = true) @RequestBody final CreateVerificationTokenInputWsDTO createVerificationTokenInputWsDTO) {
