...
public class DefaultBruteForceAttackHandler implements BruteForceAttackHandler
{
		...
		if (StringUtils.isNotEmpty(key))
		{
			...
			synchronized (attempts)
			{
				if (attempts.isAttack())
				{
					...
					final boolean afterWait = attempts.getCounter() > 0 && attempts.getLastTime().before(dateLimit);
					if (afterWait)
					{
				...
				else
				{
					...
					final boolean isAttack =
							attempts.getCounter() > 0 && maxAttempts.compareTo(attempts.getCounter()) <= 0 && attempts.getFirstTime()
									.after(dateLimit);;
					if (isAttack)
					{
	...
	protected void evict()
	{
		...
		while (cacheIterator.hasNext())
		{
			...
			if (attempts.getCounter() > 0 && attempts.getLastTime().before(dateLimit))
			{
