		...
		extends AbstractSnIndexerValueProvider<ProductModel, ProductPriceSnIndexerValueProvider.ProductPriceData>
{
	...
	{
		data.computeIfAbsent(DEFAULT_CURRENCY_KEY, key -> loadPriceValue(source));
	}

	protected Double loadPriceValue(final ProductModel source)
	{
		final PriceCriteria priceCriteria = PriceCriteria.DefaultPriceCriteria.forProduct(source).forDate(new Date());
		final List<PriceInformation> priceInformations = priceService.getPriceInformations(priceCriteria);
