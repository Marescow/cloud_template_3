...
public class ProductPriceSnIndexerValueProviderSpec extends AbstractSpockTest {
	...
	static final String QUALIFIER_TYPE_ID = "qualifierType"
  	...
  	static final long DATE_THRESHOLD_SECONDS = 15;
	...
	def "Provide value"() {
		...
		priceService.getPriceInformations({ it -> it.product == product && dateIsWithinRange(it.date) }) >> List.of(priceInformation)
	...
	def "Provide qualified value"() {
    	...
    	priceService.getPriceInformations({ it -> it.product == product && dateIsWithinRange(it.date) }) >>> [
	...
	boolean dateIsWithinRange(final Date date) {
		...
		return currentInstant.isAfter(instant.minusSeconds(DATE_THRESHOLD_SECONDS)) && currentInstant.isBefore(instant.plusSeconds(DATE_THRESHOLD_SECONDS));
	}
}
