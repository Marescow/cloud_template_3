...
public class ProductPricesValueResolverTest extends AbstractValueResolverTest
{
	...
	protected static final long DATE_THRESHOLD_SECONDS = 15;
	...
	public void resolveProductWithNoPrice() throws Exception
	{
		...
		when(getQualifierProvider().getAvailableQualifiers(getFacetSearchConfig(), getIndexedType()))
				.thenReturn(Arrays.asList(eurQualifier));
		when(priceService.getPriceInformations(argThat(it -> it.getProduct() == product && dateIsWithinRange(it.getDate()))))
				.thenReturn(priceInformations);
	...
	public void resolveWithMultipleCurrencies() throws Exception
	{
		...
		when(getQualifierProvider().getAvailableQualifiers(getFacetSearchConfig(), getIndexedType()))
				.thenReturn(Arrays.asList(eurQualifier, usdQualifier));
		when(priceService.getPriceInformations(argThat(it -> it.getProduct() == product && dateIsWithinRange(it.getDate()))))
				.thenReturn(Collections.singletonList(priceInformation));
	...
	boolean dateIsWithinRange(final Date date)
	{
		final Instant currentInstant = Instant.now();
		final Instant instant = date.toInstant();
		return currentInstant.isAfter(instant.minusSeconds(DATE_THRESHOLD_SECONDS))
				&& currentInstant.isBefore(instant.plusSeconds(DATE_THRESHOLD_SECONDS));
	}
}
