...
class AddressBookFlow extends AbstractUserTest {
	...
	def "Address book flow : #format"() {
		...
		with(restClient.patch(
				path: getBasePathWithSite() + '/users/' + customer.customerId + "/addresses/" + firstAddress.id,
				contentType: format,
				body: updates,
				requestContentType: URLENC
		)) {
		...
		HttpResponseDecorator response = restClient.patch(
				path: getBasePathWithSite() + '/users/' + customer.customerId + "/addresses/" + secondAddress.id,
				query: [
						'defaultAddress': 'true'
				],
				contentType: format,
