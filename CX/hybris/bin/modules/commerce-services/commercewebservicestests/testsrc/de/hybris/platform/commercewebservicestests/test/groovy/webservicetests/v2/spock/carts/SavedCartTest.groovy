...
class SavedCartTest extends AbstractCartTest {
	...
	def "Registered and authorized customer saves a cart and clones it with request body after: #format"() {
		given: "a registered customer with one saved cart"
		authorizeTrustedClient(restClient)
		def customer = registerCustomer(restClient)
		authorizeCustomer(restClient, customer)
		def cart = createCart(restClient, customer, format)
		def savedCart = saveCartWithRequestBody(restClient, customer, cart, format)

		when: "authorized customer wants to clone a cart"
		def response = restClient.post(
				path: getBasePathWithSite() + '/users/' + customer.id + '/carts/' + savedCart.savedCartData.code + '/copySavedCart',
				query: ['fields': FIELD_SET_LEVEL_FULL],
				body: ['name': 'clonedSaveCartName'],
				contentType: format,
				requestContentType: JSON
		)

		then: "he gets the cart copied"
		with(response) {
			if (isNotEmpty(data) && isNotEmpty(data.errors)) println(data)
			status == SC_OK
			isNotEmpty(data.savedCartData)
