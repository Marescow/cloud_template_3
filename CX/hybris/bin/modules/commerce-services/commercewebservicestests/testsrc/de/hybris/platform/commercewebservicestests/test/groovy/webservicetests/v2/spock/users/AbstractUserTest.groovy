...
abstract class AbstractUserTest extends AbstractSpockFlowTest {
	...
	protected static final GOOD_ADDRESS_DE_NO_TITLEzCODE = [
			'firstName'      : 'Jane',
			'lastName'       : 'Smith',
			'line1'          : 'Nymphenburger Str. 86 - Maillingerstrasse',
			'line2'          : 'pietro 2',
	...
	protected getAddress(RESTClient client, user, address, format, fullFields = false) {
		HttpResponseDecorator response = client.get(
				path: getBasePathWithSite() + '/users/' + user.customerId + '/addresses/' + address.id,
	...
	protected createPaymentInfo(paymentJSON, RESTClient client, customer, cartId, format = JSON) {
		HttpResponseDecorator response = client.post(
				path: getBasePathWithSite() + '/users/' + customer.customerId + '/carts/' + cartId + '/paymentdetails',
				body: paymentJSON,
				contentType: format,
				requestContentType: JSON)
		with(response) {
	...
	protected getPaymentInfo(RESTClient client, customer, infoID, format) {
		HttpResponseDecorator response = client.get(
				path: getBasePathWithSite() + '/users/' + customer.customerId + '/paymentdetails/' + infoID,
	...
	protected getOrders(RESTClient client, customer, format) {
		HttpResponseDecorator response = client.get(
				path: getBasePathWithSite() + "/users/" + customer.customerId + "/orders",
				contentType: format,
				query: ["fields": FIELD_SET_LEVEL_FULL],
				requestContentType: URLENC)
		with(response) {
