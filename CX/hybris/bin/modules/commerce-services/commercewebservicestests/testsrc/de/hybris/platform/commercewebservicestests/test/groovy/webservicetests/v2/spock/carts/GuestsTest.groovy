...
class GuestsTest extends AbstractCartTest {
	...
	private prepareCartForGuestOrder(RESTClient client, guestEmail, format) {
		...
		addEmailToAnonymousCart(client, cart.guid, guestEmail, format)
	...
	def "Guest customer logs in with request body: #format"() {
		given: "a guest customer and anonymous cart"
		authorizeClient(restClient)
		def customer = ANONYMOUS_USER
		def cart = createAnonymousCart(restClient, format)
		...
		when: "customer logs in by providing email for anonymous cart"
		def response = restClient.put(
				path: getBasePathWithSite() + '/users/' + customer.id + '/carts/' + cart.guid + "/setEmail",
				body: ['email': getGuestEmail()],
				contentType: format,
	...
	def "Guest customer places oder: #format"() {
		...
		authorizeClient(restClient)
		def guestEmail = getGuestEmail()
		def cart = prepareCartForGuestOrder(restClient, guestEmail, format)
	...
	def "Guest customer places pickup oder: #format"() {
		...
		addEmailToAnonymousCart(restClient, cart.guid, guestEmail, format) //setting email address on cart makes it recognized as guest cart
	...
	def "Guest customer creates full account after placing oder: #format"() {
		...
		def customer = ['id': guestEmail, 'password': PASSWORD]
