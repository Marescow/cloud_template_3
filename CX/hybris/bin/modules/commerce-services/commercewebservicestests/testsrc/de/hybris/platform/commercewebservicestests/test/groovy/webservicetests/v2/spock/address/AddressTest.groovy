...
class AddressTest extends AbstractUserTest {
	...
	def "Customer gets single address with cellphone and district: #format"() {
		...
		HttpResponseDecorator response = restClient.get(
				path: getBasePathWithSite() + '/users/' + customer.customerId + "/addresses/" + address.id,
				query: [
	...
	def "Customer gets non existing address : #format"() {
		...
		HttpResponseDecorator response = restClient.get(
				path: getBasePathWithSite() + '/users/' + customer.customerId + "/addresses/notExistingAddress",
				contentType: format,
				requestContentType: URLENC)
	...
	def "Customer changes his default address : #format"() {
		...
		HttpResponseDecorator response = restClient.patch(
				path: getBasePathWithSite() + '/users/' + customer.customerId + "/addresses/" + address2.id,
				query: [
						'defaultAddress': 'true'
				],
				contentType: format,
