...
import org.drools.compiler.kie.builder.customized.CustomizedKieRepository;
...
public class DefaultRuleEngineKieModuleSwapperUnitTest
{
	...
	@Mock
	private CustomizedKieRepository customizedKieRepository;
	...
	@Mock
	private ReleaseId previousReleaseId1;
	@Mock
	private ReleaseId previousReleaseId2;

	@Mock
	private KieModule kieModuleOfTest;
	...
	@Test
	public void testRemoveAllPreviousOldKieModulesIfPresentWhenEnableConfiguration()
	{
		Mockito.lenient().when(ruleEngineActionResult.getModuleName()).thenReturn(MODULE_NAME);

		Mockito.lenient().when(configuration.getBoolean("ruleengine.remove.all.previous.versions.kiemodules.in.kierepository", Boolean.FALSE))
				.thenReturn(true);
		Mockito.lenient().when(kieServices.getRepository()).thenReturn(customizedKieRepository);
		Mockito.lenient().when(customizedKieRepository.getPreviousKieModules(any(ReleaseId.class)))
				.thenReturn(asList(previousReleaseId1, previousReleaseId2));
		Mockito.lenient().when(customizedKieRepository.getKieModule(previousReleaseId1)).thenReturn(kieModuleOfTest);
		Mockito.lenient().when(customizedKieRepository.getKieModule(previousReleaseId2)).thenReturn(kieModuleOfTest);
		Mockito.lenient().when(previousReleaseId1.getVersion()).thenReturn("previousReleaseId1");
		Mockito.lenient().when(previousReleaseId2.getVersion()).thenReturn("previousReleaseId2");
		Mockito.lenient().when(ruleEngineActionResult.getDeployedVersion()).thenReturn(DEPLOYED_VERSION);
		final boolean remove = moduleSwapper.removeOldKieModuleIfPresent(ruleEngineActionResult);

		assertThat(remove).isTrue();
		verify(customizedKieRepository, times(2)).removeKieModule(any(ReleaseId.class));

	}

	@Test
	public void testRemoveOldKieModuleIfPresentWhenNotEnableConfiguration()
	{
		Mockito.lenient().when(ruleEngineActionResult.getModuleName()).thenReturn(MODULE_NAME);
		...
		Mockito.lenient().when(configuration.getBoolean("ruleengine.remove.all.previous.versions.kiemodules.in.kierepository", Boolean.FALSE))
				.thenReturn(false);
		Mockito.lenient().when(kieServices.getRepository()).thenReturn(customizedKieRepository);
		Mockito.lenient().when(customizedKieRepository.getKieModule(any(ReleaseId.class))).thenReturn(kieModuleOfTest);

		Mockito.lenient().when(ruleEngineActionResult.getDeployedVersion()).thenReturn(DEPLOYED_VERSION);
		Mockito.lenient().when(ruleEngineActionResult.getOldVersion()).thenReturn(OLD_DEPLOYED_VERSION);
		final boolean remove = moduleSwapper.removeOldKieModuleIfPresent(ruleEngineActionResult);

		assertThat(remove).isTrue();
		verify(customizedKieRepository, times(1)).removeKieModule(any(ReleaseId.class));
	...
	@Test
	public void testCopyChangesShouldRemoveDRLPath() {
		// given
		final MemoryKieModule trgKieModule = Mockito.mock(MemoryKieModule.class);
		final MemoryFileSystem trgMemoryFileSystem = Mockito.mock(MemoryFileSystem.class);
		final MemoryKieModule srcKieModule = Mockito.mock(MemoryKieModule.class);
		final MemoryFileSystem srcMemoryFileSystem = Mockito.mock(MemoryFileSystem.class);

		Mockito.lenient().when(trgKieModule.getMemoryFileSystem()).thenReturn(trgMemoryFileSystem);
		Mockito.lenient().when(srcKieModule.getMemoryFileSystem()).thenReturn(srcMemoryFileSystem);

		final Map<PortablePath, byte[]> compiledCode = new HashMap<>();
		compiledCode.put(PortablePath.of(RuleEngineConstants.DROOLS_BASE_PATH + "META-INF" + File.separator + "kmodule.xml"), new byte[0]);
		compiledCode.put(PortablePath.of(RuleEngineConstants.DROOLS_BASE_PATH + "META-INF" + File.separator + "kmodule.info"), new byte[0]);
		compiledCode.put(PortablePath.of(RuleEngineConstants.DROOLS_BASE_PATH + "pom.xml"), new byte[0]);
		final String drlFilePath = "de" + File.separator + "hybris" + File.separator + "platform" + File.separator + "test-promotion.drl";
		compiledCode.put(PortablePath.of(drlFilePath), new byte[0]);
		Mockito.lenient().when(srcMemoryFileSystem.getMap()).thenReturn(compiledCode);

		Mockito.lenient().when(trgMemoryFileSystem.existsFile(argThat(
				portablePath -> portablePath.asString().equals(RuleEngineConstants.DROOLS_BASE_PATH + drlFilePath)))).thenReturn(true);

		// when
		moduleSwapper.copyChanges(trgKieModule, srcKieModule);

		// then
		final ArgumentCaptor<PortablePath> argCaptor = ArgumentCaptor.forClass(PortablePath.class);
		verify(trgMemoryFileSystem).remove(argCaptor.capture());
		final PortablePath path = argCaptor.getValue();
		Assert.assertEquals(RuleEngineConstants.DROOLS_BASE_PATH + drlFilePath, path.asString());
	}
