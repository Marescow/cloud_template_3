...
public class CategoryWithCatalogVersionFacetDisplayNameProviderTest
{
	...
	@Test
	public void shouldReturnCategoryNameByLabelServiceForSearch() {
		//given
		final SearchQuery query = mock(SearchQuery.class);
		doReturn(LANGUAGE_EN).when(query).getLanguage();
		...
		provider.setLabelServiceProxy(null);

		//when
		final String labelValue = provider.getDisplayName(query, NAME);
		assertThat(labelValue).isNull();
		verify(labelServiceForSearch).getObjectLabel(eq(categoryMock), any());
	}
