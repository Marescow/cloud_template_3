...
export class MediaContainerComponent implements OnInit {
    ...
    public setMediaContainer(mediaContainer: MediaContainer): void {
        const allLanguages = this.getSameMediaContainerLanguages();
        if (allLanguages.length !== 0) {
            allLanguages.forEach((lang) => {
                this.model[lang] = mediaContainer;
            });
        } else {
    ...
    public onMediaContainerCreate(name: string): void {
        ...
        this.getSameMediaContainers().forEach((mediaContainer) => {
    ...
    public removeMediaByFormat(format: MediaFormatType): void {
        this.getSameMediaContainers().forEach((mediaContainer) => {
            delete mediaContainer.medias[format];
        });
    }
    ...
    private setMediaUuidForFormat(uuid: string, format: MediaFormatType): void {
        this.getSameMediaContainers().forEach((mediaContainer) => {
            mediaContainer.medias[format] = uuid;
        });
    }
    ...
    private clearModel(): void {
        this.getSameMediaContainers().forEach((mediaContainer) => {
            Object.keys(mediaContainer).forEach((key) => delete mediaContainer[key]);
        });
    }
    ...
    private resetModelForClone(): void {
        if (!!this.editor.initialContent.cloneComponent) {
            ...
            if (
                ...
                this.model[this.lang].qualifier = `${MediaContainerClonePrefix}${this.model[
                    this.lang
    ...
    /**
     * Get all media container model that has the same uuid to current component's model.
     * @private
     */
    private getSameMediaContainers(): MediaContainer[] {
        return this.getSameMediaContainerLanguages().map((lang) => this.model[lang]);
    }

    /**
     * Get all media container models' languages that has the same uuid to current component's model.
     * @private
     */
    private getSameMediaContainerLanguages(): string[] {
        if (!this.model[this.lang]) {
            return [];
        }
        const curMediaContainerUuid = this.model[this.lang]?.mediaContainerUuid;
        if (curMediaContainerUuid) {
            return Object.keys(this.model).filter(
                (curLang) =>
                    !!this.model[curLang] &&
                    this.model[curLang].mediaContainerUuid === curMediaContainerUuid
            );
        } else {
            return [this.lang];
        }
