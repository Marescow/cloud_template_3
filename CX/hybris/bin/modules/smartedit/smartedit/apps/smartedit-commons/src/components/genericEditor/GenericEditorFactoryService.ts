...
export class GenericEditorFactoryService {
    ...
    constructor(
        ...
        this.genericEditorConstructor = class implements IGenericEditor {
            ...
            onSubmit = async (payload: Payload): Promise<GenericEditorOnSubmitResponse> => {
                ...
                const acceptLangHeader = {
                    headers: {
                        'Accept-Language': await languageService.getResolveLocaleIsoCode()
                    }
                };
                ...
                return this.preparePayload(payload).then((preparedPayload: Payload) => {
                    const promise = this.editorCRUDService
                        ? this.smarteditComponentId
                            ? this.editorCRUDService.update(preparedPayload, acceptLangHeader)
                            : this.editorCRUDService.save(preparedPayload, acceptLangHeader)
