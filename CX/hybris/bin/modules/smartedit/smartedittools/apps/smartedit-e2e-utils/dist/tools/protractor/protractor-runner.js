...
const chromeDriverPattern = path.resolve(__dirname, '../../../chromedriverBin/chromedriver');
const chromeArgs = [
    'lang=en-US',
...
const buildParams = async (config) => {
    ...
    const chromedriverPath = require.resolve('../../../chromedriverBin/chromedriver');
    params.push(`--chromeDriver=${chromedriverPath}`);
    return params;
};
...
const updateWebDriver = async () => {
    try {
        ...
        const workdir = path.resolve(__dirname, '../../../../smartedit-e2e-utils');
        const output = childProcess.execSync('npm run update-driver', { cwd: workdir }).toString();
        console.log(`npm run update-driver output: ${output}`);
