...
public class AssistedServiceFacadeIntegrationTest extends ServicelayerTransactionalTest
{
	...
	{
		final PageableData pageableData = createPageableData(0, 10, "");
		final List<CustomerData> customerListWithOne = assistedServiceFacade.getCustomers("bre", pageableData);
		assertEquals(1, customerListWithOne.size());
		assertNotNull(customerListWithOne.get(0).getUid());

		final List<CustomerData> customerListWith2 = assistedServiceFacade.getCustomers("joh", pageableData);
		assertEquals(4, customerListWith2.size());

		final List<CustomerData> emptyCustomerList = assistedServiceFacade.getCustomers("afasfaf", pageableData);
		assertEquals(0, emptyCustomerList.size());
	}

	@Test
	public void testGetPagedCustomers()
	{
		PageableData pageableData = createPageableData(0, 10, "");
		final SearchPageData<CustomerData> pagedCustomersWithOne = assistedServiceFacade.getPagedCustomers("bre", pageableData);
		assertEquals(1, pagedCustomersWithOne.getResults().size());
		assertNotNull(pagedCustomersWithOne.getResults().get(0).getUid());

		pageableData = createPageableData(0, 2, "");
		final SearchPageData<CustomerData> pagedCustomersWith2 = assistedServiceFacade.getPagedCustomers("joh", pageableData);
		assertEquals(2, pagedCustomersWith2.getResults().size());
		assertEquals(4, pagedCustomersWith2.getPagination().getTotalNumberOfResults());

		final SearchPageData<CustomerData> emptyPagedCustomersList = assistedServiceFacade.getPagedCustomers("afasfaf", pageableData);
		assertEquals(0, emptyPagedCustomersList.getResults().size());
	}
