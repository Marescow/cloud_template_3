...
public class DefaultAssistedServiceService implements AssistedServiceService
{
	...
	protected StringBuilder getCustomerSearchQuery(final String searchCriteria)
	{
		...
		{
			builder.append("AND (LOWER({p:" + CustomerModel.UID + "}) LIKE CONCAT('%',  CONCAT(?username, '%')) ");
	...
	public CustomerModel createNewCustomer(final String customerId, final String customerName) throws DuplicateUidException
	{
		...
		customerModel.setPassword(generateInitialPassword());
	...
	protected String generateInitialPassword() {
		final int numberRange = 10;
		final int passwordBasicLength = 30;
		final int minimumNumber = 1;
		final SecureRandom random = new SecureRandom();
		final String randomUpperCaseLetter = generateRandomLetter(random, true, passwordBasicLength);
		final String randomLowerCaseLetter = generateRandomLetter(random, false, minimumNumber);
		final String randomNumber = String.valueOf(random.nextInt(numberRange));
		final String randomChar = generateRandomChar(random);
		final String password = randomUpperCaseLetter.concat(randomLowerCaseLetter).concat(randomNumber).concat(randomChar);
		return passwordShuffle(password);
	}

	protected String generateRandomLetter(SecureRandom random, boolean isUpperCase, int length) {
		final int letterRange = 26;
		final int ascii4A = 65;
		final int ascii4a = 97;
		final int asciiCode = isUpperCase ? ascii4A : ascii4a;
		final StringBuilder stringBuilder = new StringBuilder();
		for (int i = 0; i < length; i++) {
			final int randomInt = random.nextInt(letterRange) + asciiCode;
			stringBuilder.append((char) randomInt);
		}
		return stringBuilder.toString();
	}

	protected String generateRandomChar(SecureRandom random){
		final String chars = "!@#$%^*()_-+{};:.,";
		final int index = random.nextInt(chars.length());
		return String.valueOf(chars.charAt(index));
	}

	protected String passwordShuffle(final String password) {
		char[] passwordChars = password.toCharArray();
		List<String> randomPassword = new ArrayList<>();
		for (int i = 0; i < passwordChars.length; i++) {
			randomPassword.add(String.valueOf(passwordChars[i]));
		}
		Collections.shuffle(randomPassword);
		return randomPassword.stream().collect(Collectors.joining());
	}
